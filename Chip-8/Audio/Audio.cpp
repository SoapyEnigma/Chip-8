...
void Audio::SetupWave()
{
    ma_waveform_config wcfg =
        ma_waveform_config_init(ma_format_f32, 1, _sampleRate, ma_waveform_type_square, 0.2f, _freq);

    ma_waveform_init(&wcfg, &_wave);
}

bool Audio::Init(i32 sampleRate)
{
    _sampleRate = sampleRate;

    ma_engine_config ecfg = ma_engine_config_init();
    ecfg.sampleRate = sampleRate;

    if (ma_engine_init(&ecfg, &_engine) != MA_SUCCESS)
        return false;
